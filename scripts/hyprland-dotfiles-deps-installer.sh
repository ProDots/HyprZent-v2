...
configure_dependencies() {
    ...
    if [[ "${INSTALL_CHOICES[dev_tools]}" == "y" ]]; then
        log "  Añadiendo dependencias para 'Development Tools'..."
        case "$DISTRO_NAME" in
            ubuntu|debian|pop!_os|linuxmint)
                REQUIRED_PACKAGES+=(
    ...
    if [[ "$OS_TYPE" == "Linux" && ("$DISTRO_NAME" == "arch" || "$DISTRO_NAME" == "manjaro" || "$DISTRO_NAME" == "artix" || "$DISTRO_NAME" == "garuda") ]]; then
        if [[ "${INSTALL_CHOICES[aur_packages]}" == "y" ]]; then
            log "  Añadiendo instalación de 'AUR Helper' (paru)."
            CUSTOM_COMMANDS+=( "install_aur_helper" )
        fi
    fi

    # Filtra paquetes duplicados antes de la instalación
    REQUIRED_PACKAGES=($(printf "%s\n" "${REQUIRED_PACKAGES[@]}" | sort -u))
    log "✅ Listas de dependencias configuradas. Total de paquetes a instalar: ${#REQUIRED_PACKAGES[@]}"
    log "✅ Total de comandos personalizados a ejecutar: ${#CUSTOM_COMMANDS[@]}"
}
...
display_menu() {
    log "✨ Bienvenido al instalador de dependencias de Hyprland para tus dotfiles (v${VERSION}) ✨"
    log ""
    log "Este script te ayudará a instalar los paquetes necesarios para un entorno Hyprland completo."
    log "Selecciona las categorías que deseas instalar. Puedes optar por solo las dependencias básicas"
    log "o un conjunto más completo que incluya herramientas de desarrollo y personalización."
    log ""
    ...
    if [[ "$UNATTENDED_MODE" == "true" ]]; then
        log "✅ Modo desatendido activado. Se instalarán todas las categorías por defecto."
    ...
    log "➡️ Selecciona las categorías a instalar (s/n):"

    # Opción 1: Core Hyprland & Wayland
    while true; do
        read -rp "   1. Hyprland y Wayland Core (Librerías, gestores de sesión, audio, etc.) [s/n]: " choice
        choice=${choice,,} # Convertir a minúsculas
        if [[ "$choice" =~ ^[sn]$ ]]; then
            INSTALL_CHOICES[core_hyprland]="$choice"
            break
        else
            log "Respuesta inválida. Por favor, ingresa 's' o 'n'." >&3
        fi
    done

    # Opción 2: Eww & TiramisuPipe
    while true; do
    ...
    # Opción 3: Development Tools (Runtimes, Shell, Neovim)
    while true; do
    ...
    # Opción 4: AUR Helper (Solo para Arch-based distros)
    if [[ "$OS_TYPE" == "Linux" && ("$DISTRO_NAME" == "arch" || "$DISTRO_NAME" == "manjaro" || "$DISTRO_NAME" == "artix" || "$DISTRO_NAME" == "garuda") ]]; then
        while true; do
    ...
    if ! confirm_action "¿Estás seguro de que quieres proceder con estas selecciones?"; then
...
validate_installation() {
    log "--- Verificando la instalación de componentes clave ---"
    local all_good=true
    ...
    if [[ "$OS_TYPE" == "Linux" && ("$DISTRO_NAME" == "arch" || "$DISTRO_NAME" == "manjaro" || "$DISTRO_NAME" == "artix" || "$DISTRO_NAME" == "garuda") && "${INSTALL_CHOICES[aur_packages]}" == "y" ]]; then
        ...
        if ! check_command "yay" && ! check_command "paru"; then
            warn "Ningún AUR helper (yay/paru) encontrado."
            all_good=false
        fi
    ...
    if [[ "$all_good" == "true" ]]; then
        log "🎉 ¡Todos los componentes clave seleccionados parecen estar instalados correctamente!"
    else
        warn "Algunos componentes clave no se encontraron. Revisa el log para los detalles y posibles errores."
        log "Si algo falló, puedes intentar ejecutar el script de nuevo o instalar los componentes manualmente."
    fi
    log "--- Fin de la verificación ---"
}
...
main() {
    log "🚀 Iniciando el script de instalación de dependencias Hyprland (v${VERSION})..."
    log "Log de la sesión disponible en: '$LOG_FILE'"
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            -h|--help)
                ...
                log "  --unattended: Ejecuta la instalación sin interacción, instalando todas las categorías."
    ...
    detect_os
    check_disk_space
    display_menu # Muestra el menú de selección o configura por defecto en modo desatendido
    configure_dependencies # Configura las listas de paquetes y comandos según las selecciones
    install_system_packages
    run_custom_commands
    validate_installation

    log "✅ El script de instalación de dependencias de Hyprland ha finalizado."
    log "Puedes revisar el log completo en: '$LOG_FILE'"
    log "¡Reinicia tu sistema o sesión para asegurarte de que todos los cambios surtan efecto!"
}
